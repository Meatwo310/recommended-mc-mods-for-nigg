...
下記リンクから[ダウンロード](https://github.com/MeetingMeat/recommended-mc-mods-for-nigg/raw/main/%E3%80%90%E8%A7%A3%E5%87%8D%E3%81%97%E3%81%A6%E3%81%8A%E4%BD%BF%E3%81%84%E3%81%8F%E3%81%A0%E3%81%95%E3%81%84%E3%80%911.16.5%E6%97%A5%E6%9C%AC%E8%AA%9E%E5%8C%96%E3%83%AA%E3%82%BD%E3%83%91%20by%20Meatwo310.zip)できます。  
zipですが **解凍してお使いください。**  
https://github.com/MeetingMeat/recommended-mc-mods-for-nigg/raw/main/%E3%80%90%E8%A7%A3%E5%87%8D%E3%81%97%E3%81%A6%E3%81%8A%E4%BD%BF%E3%81%84%E3%81%8F%E3%81%A0%E3%81%95%E3%81%84%E3%80%911.16.5%E6%97%A5%E6%9C%AC%E8%AA%9E%E5%8C%96%E3%83%AA%E3%82%BD%E3%83%91%20by%20Meatwo310.zip  
...
- **Neat★**
  - MOBの頭の上に体力を表示する。configで表示距離など調整可能。
- No Nightvision Flash
  - 暗視バフが切れかけでもチカチカしなくなる(フェードアウトする)。
- Not Enough Animations
  - プレイヤーに若干のアニメーションを追加する。食事モーション、地図や松明をちゃんと持つなど。
- Not Enough Crashes
  - クラッシュ時にマイクラが落ちずにタイトルへ戻り無理やりプレイを続行できる。クラッシュの種類によっては機能しない。続行しても平気なときとちゃんと再起しないとクラッシュを繰り返す(orバグる)時がある。
- Notes
  - ゲーム内のメモ帳。ピン留めすると常時中身を表示できる。`F6`とかの割り当てがおすすめ。
- Observable
  - ワールド内のエンティティ/タイルエンティティ の負荷を測定できる。`Ctrl`+`F10`とかを割り当てるのがおすすめ。
- **Personality★**
  - sitの上位互換。どこでも座れる/クロールできる ようになる。特にクロール機能は慣れるとやみつきになる。アクセシビリティ設定からトグル/ホールド を切り替えられる。Sitは`Shift`+`無変換(スペースキーの左にある邪魔なキー)` でトグル、Crawlは`Ctrl`+`無変換` でホールド がおすすめ。
- Polymorph
  - レシピ競合時にどっちのレシピもクラフトできるようにする。クラフトEX的なMOD。
- Rebind Narrator
  - `Ctrl`+`b`で出てくる ~~クソ邪魔な~~ ナレーターを他のキーに割り当てられる(もちろん割り当てないことも可能)。
- Server Tab Info
  - `Ctrl`+`Tab`で鯖のTPS(負荷)を表示する。50を超えていたらラグい証拠。
- Shulker Tooltip
  - シュルカーボックスにカーソルを合わせると中身を表示する。いろいろキーバインドいじることをおすすめ。情報トグルは適当、情報表示は`Shift`、中身を開く(自分の環境ではクラッシュする...)は`Space`とかがおすすめ
- Shutup Experimental Settings
  - ワールドを開くときの「このワールドは試験的な機能を使用しています！」的なメッセージを無効化する。もともと出てないなら関係ないが入れといて損はない。
- Smooth Boot
  - マイクラ起動時/プレイ時 等でプロセス?の優先度を自動で調整して起動をスムーズにする(マイクラ起動中にPC自体が激重！ということがなくなる)。
- Sound Device Options
  - 音量設定画面でスピーカーデバイスの選択が出来るようになる。これがないとスピーカー/ヘッドフォン/ワイヤレスイヤホン などを切り替えたときにいちいち`F3`+`T`しないといけない(環境による)。
- Sound Filters
  - 洞窟などで音が反響するようになったり、羊毛で音をある程度遮ったりできる(ExtremeSoundMufflerがあればいらない)。ヘッドフォンつけてるなら一層おすすめ。
- Toast Control
  - 右上に出てくるレシピアンロックや実績解除の通知を無効化できる。configで何を無効化するか調整可能。旧バージョンのマイクラの通知表示アニメーション(右から左ではなく上から下)に変更できたりする。
- **ToroHealth(導入済み)★★**
  - NeatやJadeがあるといらない子のように見えるが、`{起動構成}/config/torohealth.json`をいじって、scale(大きさ)を0にすると、敵への与ダメを表示するだけのMODにできる(はず)。地味だけどとても便利。
- Too Fast
  - チート対策用に設けられている「早く動きすぎるとロルバする」機能を無効化する。マルチじゃないなら入れておくべし。

- ModernUI
  - (注:筆者は使ったことがありません) フォントとかUIをきれいにするMOD? 多分便利なんだと思う。


## 中・大規模(バニラの雰囲気を 軽く/大きく 破壊します)
- AngelBlock
  - 空中に設置できるブロックを追加する。
- Antique Atlas
  - バニラの地図以上JourneyMap未満なスペックの地図MOD。スクロールやウェイポイント機能が付いた地図の本を追加する。農業という雰囲気にはあっているはず。
- **BetterNether / BetterEnd ★**
  - ネザーやエンドをなんかもうすごいことにする(語彙力皆無)。CurseForgeのトレーラー?動画を見たほうが早い。
- Better Than Mending
  - 修繕の付いたツールを`Shift`+`右クリック`すると、自身の経験値を消費してツールを修繕できる。
- Bottle Your XP
  - 空瓶を持って`Shift`+`右クリック`で、自身の経験値を瓶に詰められる。configで変えられるが詰めたときにダメージを受ける。
- **Bow Infinity Fix★**
  - 無限の付いた弓が矢を所持していないときも使えるようになる(バニラは最低1本必要)
- Building Gadgets
  - 大規模な建築をするときに便利なガジェットを追加する。電力を必要とするがconfigでオフにできたかな...? 農業という雰囲気に合わない気がするので要注意。
- Ceiling Torch
  - 松明を天井に逆さで設置できる。
- Chisel
  - オシャレな建築ブロックを追加する。Chisel(彫刻刀/チゼル)で丸石も美しい建材に加工できる。
- **Chunk Loaders★**
...
- Redstone Gates
  - AND回路やOR回路などを1ブロック内で完結させられる...らしい。
- Ring of Teleport
